...
Breadcrumbs::for('category_admin_home', function (BreadcrumbTrail $trail, Category $category) {
    if ($category->parent_id !== 0 && $category->parentCategory) {
        $trail->parent('category_admin_home', $category->parentCategory);
    } 
...
Breadcrumbs::for('product_admin', function (BreadcrumbTrail $trail, Product $product) {
    $trail->parent('category_admin_home', $product->category);
});

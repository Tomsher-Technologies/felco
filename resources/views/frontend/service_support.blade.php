            ...
            <div class="row">
                <div class="col-md-8 m-auto">
                    ...
                    <div class="accordion mb-5" id="accordionExample">
                        ...
                        @php
                            $query = \App\Models\Faq::where('type','service_support')->where('lang', $lang)->orderBy('sort_order','asc');
                            ...
                            $faqs = $query->get();
                            if(empty($faqs[0])){
                                $query = \App\Models\Faq::where('type','service_support')->where('lang', 'en')->orderBy('sort_order','asc');
                                if(request('keyword') != NULL){
                                    $query = $query->where(function ($subQuery) {
                                            $subQuery->where('question', 'LIKE', '%' . request('keyword') . '%')
                                                    ->orWhere('answer', 'LIKE', '%' . request('keyword') . '%');
                    ...
                    <h5 style="text-align: center; font-weight: 300;">{{ trans('messages.or') }}</h5>

                    <div class="row">
                        <div class="col-md-12">
                          <div class="panel panel-default ">
                             <div class="panel-body">
                                <h3>{{ $page->getTranslation('sub_title', $lang) }}</h3>
                                <a href="{{ route('contact') }}" class="theme-btn fl-get-in-touch-icon">{{ trans('messages.contact_us') }}</a>
                             </div>
                           </div>
                        </div>
                    </div>

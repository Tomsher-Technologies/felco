                ...
                @foreach ($categories as $cat)
                    <a href="{{ route('products.category', ['category_slug' => $cat->slug]) }}"
                       class="group relative bg-white border !border-slate-50/10 shadow hover:shadow-xl overflow-hidden transition-all duration-300 flex flex-col">
                        ...
                        {{-- Arrow Action --}}
                        <div class="absolute bottom-6 right-6 z-10 text-white bg-[#f16c31] p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform scale-75 group-hover:scale-100 shadow-lg">

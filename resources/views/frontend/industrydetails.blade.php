@extends('frontend.layouts.app')

@section('content')

{{-- 1. Title & Hero Section --}}
@include('components.page-title', [
    'title' => $page->getTranslation('title', $lang),
    'description' => $page->getTranslation('content', 'en'),
    'image' => getPageImage($page->type),
    'theme' => 'dark'
])

{{-- 2. Products Used Section (Dark Theme) --}}
<section class="py-16 md:py-24 bg-[#2a2a2a] text-slate-300 mt-16">
         <x-container>



<div class="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-10 items-start">

{{-- Left Column: Sticky Image --}}
    {{-- Sticky Image --}}
    <div id="industryStickyImage" class="lg:col-span-2 lg:sticky lg:top-24 self-start">
        <div class="relative h-full max-h-[800px] overflow-hidden shadow-2xl shadow-black/30">
            <img src="{{ getPageImage($page->type) }}"
                 alt="{{ $page->getTranslation('title', $lang) }}"
                 class="w-full h-full object-cover">
        </div>
    </div>

{{-- Right Column: Scroll Content --}}
    <div id="industryScrollContent" class="lg:col-span-3 space-y-16">
                {{-- Section Header --}}
                <div>
                    
                <div class="mb-8 pl-4 border-l-2 border-orange-500">
             <h3 class="text-3xl font-light text-white animate-on-scroll" >
    Products Used
</h3>
<p class="mt-2 text-slate-400 animate-on-scroll" >
    Our range of electric motors, engineered for industrial demands with proven efficiency and reliability.
</p>

                    </div>

                    {{-- Grid --}}
        <div id="industryProductGrid" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {{-- IEC Motors Card --}}
                        <div class="group bg-slate-800/50 border border-slate-700  p-6 transition-all duration-300 hover:border-orange-500/80 hover:bg-slate-800">
                            <div class="text-orange-500 mb-4">
                         


                                <svg class="w-[60px] h-[60px] fill-orange-500" xmlns="http://www.w3.org/2000/svg" id="fi_17447069" data-name="Layer 1" viewBox="0 0 64 64"><path d="M33.952,19.548H21.091a1,1,0,0,0,0,2H33.952a1,1,0,0,0,0-2Z"></path><path d="M33.952,25.274H21.091a1,1,0,0,0,0,2H33.952a1,1,0,0,0,0-2Z"></path><path d="M33.952,31H21.091a1,1,0,0,0,0,2H33.952a1,1,0,0,0,0-2Z"></path><path d="M33.952,36.726H21.091a1,1,0,0,0,0,2H33.952a1,1,0,0,0,0-2Z"></path><path d="M33.952,42.452H21.091a1,1,0,0,0,0,2H33.952a1,1,0,0,0,0-2Z"></path><path d="M61,28.276H54.277V26.922a2.566,2.566,0,0,0-2.563-2.563H49.7V20.666a5.394,5.394,0,0,0-5.383-5.394H43.34a2.159,2.159,0,0,0-2.118-1.76H39.036a2.168,2.168,0,0,0-2.129,1.76H18.136a2.168,2.168,0,0,0-2.129-1.76H13.821a2.16,2.16,0,0,0-2.119,1.76H7.383A5.394,5.394,0,0,0,2,20.666V43.345a5.389,5.389,0,0,0,5.383,5.383H11.7a2.161,2.161,0,0,0,2.119,1.76h2.186a2.168,2.168,0,0,0,2.129-1.76H36.907a2.168,2.168,0,0,0,2.129,1.76h2.186a2.159,2.159,0,0,0,2.118-1.76h.975A5.389,5.389,0,0,0,49.7,43.345V39.639h2.016a2.566,2.566,0,0,0,2.563-2.563V35.724H61a1,1,0,0,0,1-1V29.276A1,1,0,0,0,61,28.276ZM4,43.345V20.666a3.392,3.392,0,0,1,3.383-3.394h4.279V46.728H7.383A3.387,3.387,0,0,1,4,43.345Zm12.176,4.984a.168.168,0,0,1-.169.159H13.821a.167.167,0,0,1-.159-.159V15.671a.167.167,0,0,1,.159-.159h2.186a.168.168,0,0,1,.169.159V48.329Zm2-31.057h18.69V46.728H18.176Zm23.2,31.057a.158.158,0,0,1-.158.159H39.036a.169.169,0,0,1-.17-.159V15.671a.169.169,0,0,1,.17-.159h2.186a.158.158,0,0,1,.158.159V48.329ZM47.7,43.345a3.387,3.387,0,0,1-3.383,3.383H43.38V17.272h.935A3.392,3.392,0,0,1,47.7,20.666Zm4.579-6.269a.563.563,0,0,1-.563.563H49.7V26.359h2.016a.564.564,0,0,1,.563.563ZM60,33.724H54.278V30.276H60Z"></path></svg>







                            </div>
                            <h4 class="text-xl font-normal text-slate-100">IEC Motors</h4>
                            <p class="text-slate-400 text-sm mt-1">High-efficiency IEC three-phase motors for long-lasting industrial performance.</p>
                        </div>

                        {{-- Special Motors Card --}}
                        <div class="group bg-slate-800/50 border border-slate-700  p-6 transition-all duration-300 hover:border-orange-500/80 hover:bg-slate-800">
                            <div class="text-orange-500 mb-4">
                      <svg class="w-[60px] h-[60px] fill-orange-500"   viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" id="fi_2479505"><g id="dc_motor-electronics-2" data-name="dc motor-electronics"><path d="m61 29h-2v-3a1 1 0 0 0 -1-1h-2v-4h4v-2h-4v-1a3.009 3.009 0 0 0 -3-3h-31a3.009 3.009 0 0 0 -3 3v7h-3a1 1 0 0 0 -1 1v3h-4v-2a1 1 0 0 0 -1-1h-7a1 1 0 0 0 -1 1v10a1 1 0 0 0 1 1h7a1 1 0 0 0 1-1v-2h4v3a1 1 0 0 0 1 1h3v7a3.009 3.009 0 0 0 3 3h31a3.009 3.009 0 0 0 3-3v-1h4v-2h-4v-4h2a1 1 0 0 0 1-1v-3h2a1 1 0 0 0 1-1v-4a1 1 0 0 0 -1-1zm-52 7h-5v-1h5zm0-3h-5v-2h5zm0-4h-5v-1h5zm6 4h-4v-2h4zm4 4h-2v-10h2zm27 2h-21v-14h21zm0-16h-22a1 1 0 0 0 -1 1v16a1 1 0 0 0 1 1h22v6h-24a1 1 0 0 1 -1-1v-28a1 1 0 0 1 1-1h24zm8 23a1 1 0 0 1 -1 1h-5v-30h5a1 1 0 0 1 1 1zm3-9h-1v-10h1zm3-4h-1v-2h1z"></path></g></svg>

                            </div>
                            <h4 class="text-xl font-normal text-slate-100">Special Motors</h4>
                            <p class="text-slate-400 text-sm mt-1">Purpose-built motors including dual-speed, inverter-duty, and brake motors.</p>
                        </div>

                        {{-- Custom Solutions Card --}}
                        <div class="group md:col-span-2 bg-slate-800/50 border border-slate-700  p-6 transition-all duration-300 hover:border-orange-500/80 hover:bg-slate-800 flex items-center">
                            <div class="text-orange-500 mr-6">
                      

                                <svg class="w-[60px] h-[60px] fill-orange-500" id="fi_4205637"  viewBox="0 0 74 74"  xmlns="http://www.w3.org/2000/svg"><path d="m63.361 33.2a1 1 0 0 1 -.369-.071l-7.54-3a1 1 0 0 1 -.047-1.839 3.189 3.189 0 0 0 1.627-1.711 3.175 3.175 0 0 0 -1.783-4.124 3.18 3.18 0 0 0 -4.128 1.774 3.068 3.068 0 0 0 .008 2.349 1 1 0 0 1 -1.289 1.322l-7.569-3.008a1 1 0 0 1 -.56-1.3l2.332-5.865a5.2 5.2 0 0 1 -1.9-.371 5.177 5.177 0 1 1 5.457-8.593l2.338-5.873a1 1 0 0 1 1.3-.559l18.892 7.509a2.971 2.971 0 0 1 1.662 3.852l-7.5 18.871a1 1 0 0 1 -.931.637zm-5.5-4.262 4.939 1.962 7.131-17.942a.98.98 0 0 0 -.541-1.255l-17.962-7.144-2.628 6.625a1 1 0 0 1 -1.842.041 3.177 3.177 0 1 0 -1.693 4.262 1 1 0 0 1 1.31 1.294l-2.636 6.619 4.952 1.969a5.186 5.186 0 1 1 8.971 3.566z"></path><path d="m59.9 43.271a1 1 0 0 1 -.009-2h.009a1 1 0 1 1 0 2z"></path><path d="m49.529 43.271a1 1 0 0 1 -.01-2h.01a1 1 0 1 1 0 2z"></path><path d="m49.519 43.276a1 1 0 0 1 -1-.994 1 1 0 1 1 1.4.91.98.98 0 0 1 -.4.084z"></path><path d="m57.369 71.74h-51.842a3.526 3.526 0 0 1 -3.527-3.518v-51.851a3.522 3.522 0 0 1 3.527-3.508h25.911a1 1 0 0 1 1 1v10.368a1 1 0 0 1 -1.7.718 4.285 4.285 0 0 0 -3-1.205 4.328 4.328 0 0 0 0 8.656 4.285 4.285 0 0 0 3-1.2 1 1 0 0 1 1.7.718v9.358h7.49a6.339 6.339 0 1 1 11.478 0h8.471a1 1 0 0 1 1 1v25.946a3.516 3.516 0 0 1 -3.508 3.518zm-51.842-56.877a1.52 1.52 0 0 0 -1.527 1.508v51.851a1.525 1.525 0 0 0 1.527 1.518h51.842a1.515 1.515 0 0 0 1.508-1.518v-24.951h-9.348a1 1 0 0 1 -.722-1.691 4.328 4.328 0 1 0 -7.467-2.986 4.193 4.193 0 0 0 1.2 2.974 1 1 0 0 1 -.711 1.7h-10.391a1 1 0 0 1 -1-1v-8.468a6.328 6.328 0 1 1 0-11.464v-7.473z"></path><path d="m31.448 53.655a1 1 0 0 1 -1-1v-10.373a1 1 0 1 1 2-.011v10.379a1 1 0 0 1 -1 1.005z"></path><path d="m31.455 71.7a1 1 0 0 1 -1-1v-10.384a1 1 0 0 1 2 0v10.384a1 1 0 0 1 -1 1z"></path><path d="m31.448 43.271h-10.348a1 1 0 0 1 0-2h10.348a1 1 0 0 1 0 2z"></path><path d="m13.415 43.277h-10.387a1 1 0 1 1 0-2h10.387a1 1 0 0 1 0 2z"></path><path d="m17.237 52.309a6.323 6.323 0 0 1 -4.537-10.729 1 1 0 0 1 1.434 1.394 4.323 4.323 0 0 0 3.1 7.335 4.336 4.336 0 0 0 4.336-4.328 4.249 4.249 0 0 0 -1.21-3 1 1 0 1 1 1.426-1.4 6.243 6.243 0 0 1 1.784 4.405 6.339 6.339 0 0 1 -6.333 6.323z"></path><path d="m35.16 62.83a6.308 6.308 0 0 1 -4.4-1.786 1 1 0 1 1 1.388-1.439 4.324 4.324 0 1 0 .036-6.263 1 1 0 0 1 -1.73-.678v-.009a1 1 0 0 1 .306-.719 6.333 6.333 0 1 1 4.4 10.894z"></path></svg>






                            </div>

                            <div>
                                <h4 class="text-xl font-normal text-slate-100">Custom Solutions</h4>
                                <p class="text-slate-400 text-sm mt-1">Fully tailored motor solutions to meet your application's unique mechanical and electrical needs.</p>
                            </div>
                        </div>
                    </div>
                </div>






            </div>
        </div>




    </x-container>


</section>






{{-- 3. Applications Section (Light Theme) --}}
<section id="applicationSection" class="py-16 md:py-24 bg-white text-slate-800">
         <x-container>

        {{-- Section Header --}}
        <div id="applicationHeader" class="mb-12 pl-4 border-l-2 border-[#0a8268]">
            <h3 class="text-3xl font-light text-slate-900 animate-on-scroll">Applications</h3>
            <p class="mt-2 text-slate-600 animate-on-scroll">
                Discover where our motors deliver value, from smart factories to mission-critical industrial equipment.
            </p>
        </div>

        {{-- Grid --}}
      <div id="applicationGrid" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            {{-- Automation & CNC --}}
            <div class="application-card group bg-gray-100 border border-gray-300  p-6 transition-all duration-300 hover:border-[#0a8268] hover:bg-gray-50">
                <div class="text-[#0a8268] mb-4">
         
                    <svg class="w-[60px] h-[60px] fill-[#0a8268]" id="fi_2043061" enable-background="new 0 0 124 124" height="512" viewBox="0 0 124 124" width="512" xmlns="http://www.w3.org/2000/svg"><path d="m62.082 79.883c-9.972 0-18.085-7.951-18.085-17.726s8.113-17.727 18.085-17.727c1.414 0 3.137.348 3.023 2.158-.109 1.728-1.675 1.842-3.023 1.842-7.767 0-14.085 6.158-14.085 13.727s6.318 13.726 14.085 13.726 14.085-6.158 14.085-13.726c0-2.212-.522-4.322-1.552-6.273-.516-.976-.142-2.186.834-2.702s2.188-.144 2.703.835c1.318 2.496 2.015 5.311 2.015 8.14 0 9.775-8.112 17.726-18.085 17.726z"></path><path d="m71.62 51.494c-.405 0-.814-.123-1.168-.378-.529-.381-1.091-.73-1.67-1.037-.977-.517-1.349-1.726-.833-2.703.516-.975 1.726-1.35 2.703-.832.742.393 1.462.839 2.14 1.328 1.536 1.107.724 3.622-1.172 3.622z"></path><path d="m116.474 30.605-.071-8.683c-.009-1.099-.903-1.983-2-1.983-.006 0-.011 0-.017 0-1.104.009-1.993.912-1.984 2.016l.021 2.635c-12.007-14.298-31.24-23.59-49.688-23.59-19.076 0-36.734 8.544-48.439 23.439-.682.868-.531 2.125.337 2.807.869.683 2.126.532 2.809-.337 10.942-13.924 27.451-21.909 45.293-21.909 17.934 0 36.663 9.366 47.813 23.623h-5.489c-1.104 0-2 .895-2 2 0 1.104.896 2 2 2h9.415c.645-.001 2-.72 2-2.018z"></path><path d="m119.937 39.766c-.4-1.028-1.56-1.538-2.589-1.137-1.029.4-1.539 1.56-1.138 2.59 2.515 6.453 3.79 13.264 3.79 20.244 0 21.931-15.277 42.99-37.045 52.057l1.49-2.28c.604-.924.344-2.164-.58-2.767-.925-.603-2.164-.345-2.769.58l-4.77 7.298c-.664 1.013-.237 2.436.867 2.924l8.036 3.544c2.359 1.035 3.974-2.618 1.614-3.66l-3.419-1.507c10.495-4.167 20.009-11.097 27.169-19.876 8.772-10.756 13.407-23.312 13.407-36.313 0-7.479-1.367-14.778-4.063-21.697-.401-1.029 2.696 6.919 0 0z"></path><path d="m65.108 117.878c-.787.031-1.578.047-2.373.047-31.576 0-57.265-25.328-57.265-56.462 0-5.508 2.315-12.397 4.521-18.212l1.764 3.903c1.065 2.354 4.706.699 3.646-1.646l-3.729-8.25c-.459-1.014-1.722-1.463-2.718-.965l-7.849 3.931c-.988.494-1.387 1.696-.893 2.683.495.988 1.699 1.389 2.684.892l3.221-1.613c-2.282 6.05-4.646 13.222-4.646 19.276 0 33.338 27.483 60.461 61.265 60.461.848 0 1.692-.018 2.531-.051 1.104-.043 1.963-.974 1.919-2.077s-.977-1.959-2.078-1.917c-.786.031 1.101-.042 0 0z"></path><path d="m65.201 101.99c-1.963 0-7.782.023-8.688-.363-1.56-.664-2.832-2.357-2.832-4.093v-3.984c-3.014-.79-5.925-2.02-8.561-3.617l-2.8 2.794c-1.758 1.736-4.755 1.445-6.442-.169-.824-.789-4.032-4.065-4.774-4.927-1.582-1.839-1.547-4.689.182-6.415l2.818-2.811c-1.648-2.808-2.85-5.759-3.534-8.678h-3.944c-2.459 0-4.491-2.02-4.626-4.457-.063-1.139-.033-5.734 0-6.878.068-2.357 1.975-4.621 4.462-4.621h3.998c.791-3.005 2.022-5.908 3.622-8.535l-2.795-2.789c-1.749-1.749-1.446-4.759.169-6.439.791-.822 4.075-4.024 4.938-4.763 1.84-1.575 4.69-1.54 6.418.182l2.822 2.816c2.815-1.646 5.775-2.846 8.703-3.53v-4.093c0-2.46 2.028-4.485 4.462-4.621 1.141-.063 5.747-.036 6.894 0 2.493.078 4.462 2.115 4.462 4.621v3.984c3.014.79 5.925 2.019 8.561 3.617l2.8-2.794c1.754-1.742 4.759-1.441 6.442.17.824.789 4.032 4.064 4.773 4.926 1.582 1.84 1.547 4.689-.183 6.415l-2.817 2.811c1.648 2.807 2.849 5.757 3.534 8.677h3.944c2.402 0 4.529 1.777 4.775 4.209.119 1.177.05 5.95.016 7.127-.068 2.358-1.973 4.62-4.462 4.62h-3.998c-.791 3.005-2.022 5.907-3.622 8.536l2.795 2.788c1.75 1.75 1.446 4.76-.169 6.44-.791.822-4.074 4.023-4.938 4.762-1.841 1.576-4.689 1.543-6.418-.181l-2.822-2.816c-2.815 1.645-5.774 2.846-8.703 3.53v3.929c0 2.358-2.026 4.62-4.462 4.62zm-7.521-4.615c.043.203.186.615.626.615h6.737c.205-.043.62-.185.62-.621v-5.568c0-.977.707-1.812 1.672-1.972 3.409-.567 6.949-1.995 10.238-4.128.791-.513 1.834-.403 2.501.262l3.939 3.931c.317.317.742.13.942.012l4.792-4.622c.286-.285.113-.69-.005-.892l-3.794-3.786c-.683-.681-.782-1.752-.237-2.547 2.023-2.951 3.546-6.533 4.288-10.088.194-.927 1.011-1.591 1.958-1.591h5.424c.204-.042.619-.183.619-.619v-6.726c-.082-.405-.348-.61-.791-.61h-5.581c-.978 0-1.812-.706-1.973-1.671-.567-3.399-1.998-6.929-4.135-10.21-.517-.793-.407-1.839.263-2.508l3.939-3.931c.311-.31.128-.729.012-.928l-4.632-4.781c-.29-.288-.702-.114-.907.004l-3.795 3.787c-.68.679-1.747.778-2.54.235-2.957-2.017-6.549-3.538-10.114-4.279-.928-.192-1.593-1.011-1.593-1.958v-5.568c0-.385-.269-.621-.462-.621h-6.737c-.205.043-.62.185-.62.621v5.732c0 .977-.707 1.812-1.672 1.972-3.408.567-6.948 1.994-10.238 4.127-.791.515-1.833.404-2.5-.262l-3.939-3.93c-.316-.316-.742-.13-.942-.013l-4.792 4.622c-.286.285-.113.69.005.892l3.794 3.787c.683.682.782 1.752.237 2.547-2.022 2.949-3.545 6.532-4.288 10.087-.194.927-1.011 1.591-1.958 1.591h-5.421c-.205.044-.62.185-.62.622v6.72c.043.203.186.615.626.615h5.581c.978 0 1.812.707 1.973 1.671.567 3.398 1.997 6.929 4.135 10.211.517.793.407 1.839-.263 2.508l-3.939 3.93c-.311.311-.128.73-.012.929l4.632 4.781c.29.289.702.115.907-.005l3.795-3.787c.679-.679 1.747-.778 2.541-.235 2.956 2.018 6.547 3.539 10.113 4.28.928.192 1.593 1.011 1.593 1.958v5.408z"></path></svg>



                </div>
                <h4 class="text-xl font-normal text-slate-900">Automation & CNC</h4>
                <p class="text-slate-600 text-sm mt-1">
                    Powering assembly lines, robotic systems, and CNC machines that require precision and speed.
                </p>
            </div>

            {{-- Conveyors & Gearboxes --}}
            <div class="application-card group bg-gray-100 border border-gray-300  p-6 transition-all duration-300 hover:border-[#0a8268] hover:bg-gray-50">
                <div class="text-[#0a8268] mb-4">
      

<svg class="w-[60px] h-[60px] fill-[#0a8268]"  xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 128 128" id="fi_3225632"><g><path d="M126.645,54.358l-9.979-2.315a53.474,53.474,0,0,0-6.99-16.807l5.436-8.72a1.75,1.75,0,0,0-.247-2.163l-11.22-11.22a1.749,1.749,0,0,0-2.163-.248L92.76,18.32a53.48,53.48,0,0,0-16.806-6.989L73.638,1.352A1.75,1.75,0,0,0,71.934,0H56.066a1.75,1.75,0,0,0-1.7,1.355l-2.316,9.979A53.48,53.48,0,0,0,35.24,18.32l-8.722-5.435a1.749,1.749,0,0,0-2.163.248l-11.22,11.22a1.75,1.75,0,0,0-.247,2.163l5.436,8.72a53.474,53.474,0,0,0-6.99,16.807l-9.98,2.315A1.751,1.751,0,0,0,0,56.064V71.931a1.751,1.751,0,0,0,1.354,1.7l9.98,2.315a53.474,53.474,0,0,0,6.99,16.807l-5.436,8.72a1.752,1.752,0,0,0,.247,2.164l11.22,11.219a1.749,1.749,0,0,0,2.163.248l8.722-5.435a53.456,53.456,0,0,0,16.806,6.989l2.316,9.98a1.75,1.75,0,0,0,1.7,1.354H71.934a1.75,1.75,0,0,0,1.7-1.354l2.316-9.98a53.456,53.456,0,0,0,16.806-6.989l8.722,5.435a1.749,1.749,0,0,0,2.163-.248l11.22-11.219a1.752,1.752,0,0,0,.247-2.164l-5.436-8.72a53.474,53.474,0,0,0,6.99-16.807l9.979-2.315a1.752,1.752,0,0,0,1.355-1.7V56.064A1.752,1.752,0,0,0,126.645,54.358ZM124.5,70.54l-9.706,2.252a1.75,1.75,0,0,0-1.319,1.354,50.063,50.063,0,0,1-7.331,17.629,1.75,1.75,0,0,0-.026,1.891l5.289,8.484-9.254,9.253-8.484-5.288a1.752,1.752,0,0,0-1.891.026,50.018,50.018,0,0,1-17.629,7.331,1.75,1.75,0,0,0-1.354,1.319L70.543,124.5H57.457l-2.252-9.706a1.752,1.752,0,0,0-1.354-1.319,50.01,50.01,0,0,1-17.629-7.331,1.752,1.752,0,0,0-1.891-.026L25.847,111.4l-9.254-9.253,5.289-8.484a1.75,1.75,0,0,0-.026-1.891,50.044,50.044,0,0,1-7.331-17.629,1.75,1.75,0,0,0-1.319-1.354L3.5,70.54V57.454L13.206,55.2a1.749,1.749,0,0,0,1.319-1.353,50.041,50.041,0,0,1,7.331-17.63,1.75,1.75,0,0,0,.026-1.891l-5.289-8.484,9.254-9.253,8.484,5.288a1.75,1.75,0,0,0,1.891-.026,50.01,50.01,0,0,1,17.629-7.331A1.752,1.752,0,0,0,55.205,13.2L57.457,3.5H70.543L72.8,13.2a1.75,1.75,0,0,0,1.354,1.319,50.018,50.018,0,0,1,17.629,7.331,1.75,1.75,0,0,0,1.891.026l8.484-5.288,9.254,9.253-5.289,8.484a1.75,1.75,0,0,0,.026,1.891,50.06,50.06,0,0,1,7.331,17.63,1.749,1.749,0,0,0,1.319,1.353l9.706,2.252Z"></path><path d="M64,30A34,34,0,1,0,98,64,34.038,34.038,0,0,0,64,30Zm0,64.5A30.5,30.5,0,1,1,94.5,64,30.534,30.534,0,0,1,64,94.5Z"></path></g></svg>
                    
                </div>
                <h4 class="text-xl font-normal text-slate-900">Conveyors & Gearboxes</h4>
                <p class="text-slate-600 text-sm mt-1">
                    Driving conveyor systems and gearboxes in manufacturing and packaging plants.
                </p>
            </div>
        </div>
</x-container>
</section>

@endsection

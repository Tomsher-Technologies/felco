...
@include('components.page-title', [
    ...
    'image' => asset('assets/images/page/manuals.jpg') // optional
])
    ...
    @foreach ($manuals as $key => $man)
        <section class="py-16 @if((($key + 1) % 2) == 0) bg-white @else bg-gray-50 @endif">
            <x-container>
                ...
                <!-- Section Title -->
                <div class="mb-12 animate-on-scroll"><!-- ✅ animation class added here -->
                    <h2 class="text-3xl md:text-4xl font-light text-slate-800 tracking-tight mb-3">
                        {{ $man->getTranslation('title', $lang) }}
                    </h2>
                ...
                <div class="grid md:grid-cols-2 gap-10 items-start">
                    ...
                    <!-- Left: Image -->
                    <div class="overflow-hidden  shadow-md animate-on-scroll"><!-- ✅ -->
                        <img src="{{ uploaded_asset($man->image) }}"
                             alt="{{ $man->getTranslation('title', $lang) }}"
                             class="w-full h-auto object-cover transition-transform duration-500 hover:scale-105">
                    </div>

                    <!-- Right: Sections -->
                    <div class="space-y-6">
                        @foreach ($man->sections ?? [] as $section)
                            <div class="group bg-white border border-slate-200 hover:border-[#f06425]  shadow-sm p-6 transition duration-300 animate-on-scroll"><!-- ✅ -->
                                <!-- Section Title -->
                                <h5 class="animate-on-scroll text-lg font-semibold text-slate-800 mb-2">

    ...
    @if (!empty($certificates[0]))
        @foreach ($certificates as $key => $cert)
            <section class="certificate_area @if((($key+1) % 2) == 0)  bg-white mt-0 @endif">
                <div class="container">
                    <div class="row">
                        <div class="col-md-6 dd-sticky">
                            <img src="{{ uploaded_asset($cert->image) }}" class="img-fluid" alt="Brochure Image">
                            <h3>{{ $cert->getTranslation('title', $lang) }}</h3>
                        </div>
                        <div class="col-md-6">
                            <div class="row g-3">
                                @if (!empty($cert->sections[0]))
                                    @foreach ($cert->sections as $sections)
                                        <div class="col-md-12">

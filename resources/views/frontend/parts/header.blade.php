                                ...
                                <div x-show="openDropdown === 'industries'" class="px-6 py-8">
                                    ...
                                    <nav aria-label="All industries list" class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                        @foreach($industriesPages as $pdt)
                                            <a href="{{ route('industry.details', ['type' => $pdt->type]) }}" class="group relative flex items-center px-4 py-3 bg-white border border-slate-200 hover:border-[#f06425] hover:bg-orange-50 text-md font-normal text-slate-800 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#f06425]">
                                                <div class="absolute left-0 top-0 h-full w-1 bg-[#f06425] scale-y-0 group-hover:scale-y-100 transition-transform origin-top duration-300 ease-in-out"></div>
                                    ...
                                    <div class="mt-6 py-4 pl-5 border-t border-slate-200 bg-gradient-to-r from-orange-50 via-white to-white">
                                        <a href="{{ url('/industries') }}" class="group inline-flex items-center gap-2 font-normal text-[#f06425] text-base transition-all duration-300 hover:gap-3">
                                            View All Industries
                        ...
                        <div x-show="langSwitcherOpen" x-transition class="origin-top-right absolute right-0 mt-2 w-40 shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" x-cloak>
                            <div class="py-1">
                                @foreach ($availableLangs as $code => $langData)
                                    <a href="{{ $langData['route'] }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{{ $langData['name'] }}</a>
                                 @endforeach
                            </div>
                        </div>
                    </div>
        ...
        <ul class="flex flex-col mt-4 font-medium space-y-2 p-4">
            ...
            <li>
                ...
                <div x-show="openDropdown === 'industries'" x-collapse.duration.300ms class="pl-4 mt-2 space-y-2">
                    @foreach($industriesPages as $pdt)
                        <a href="{{ route('industry.details', ['type' => $pdt->type]) }}" class="block py-1 text-sm text-gray-600 hover:text-orange-500">{{ $pdt->getTranslation('title', $lang) }}</a>
                    @endforeach
                </div>
            </li>
            <li>

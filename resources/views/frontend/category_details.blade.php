...
{{-- 2. Key Specs Section (Icons Removed) --}}
...
<section class="bg-gradient-to-tr from-stone-50 via-stone-100 to-stone-50 py-10 md:py-16">
    <x-container class="mx-auto max-w-4xl">
        ...
        {{-- A single container for both description blocks --}}
        <div class="animate-on-scroll relative overflow-hidden rounded-lg bg-white p-8 shadow-lg md:p-12">
            ...
            <div>
                <h3 class="animate-on-scroll relative mb-6 text-3xl font-light tracking-wide text-stone-900 drop-shadow-sm">
                    {{ $category->getTranslation('title1', $lang) }}
                </h3>
                <div class="prose animate-on-scroll prose-lg max-w-none leading-relaxed text-stone-700">
            ...
            {{-- A top margin is added here only if the first block also exists --}}
            <div class="@if($category->getTranslation('title1', $lang)) mt-12 @endif">
                <h3 class="animate-on-scroll relative mb-6 text-3xl font-light tracking-wide text-stone-900 drop-shadow-sm">
...
<section class="bg-white py-16 md:py-20">
    <x-container>
        ...
        <div class="mb-12">
            ...
            <div class="pt-6">
                <form id="live-filter-form" class="flex flex-wrap items-end gap-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm" method="GET" action="{{ route('products.category', $category->slug) }}">
                    @php
                        $filterOptions = ['frame_size' => $frameSizes, 'poles' => $poles, 'power' => $powers, 'mounting' => $mountings, 'voltage' => $voltages];
                    @endphp
                    @foreach ($filterOptions as $filter => $options)
                        @if(collect($options)->isNotEmpty())
                        <div class="min-w-[150px] flex-1">
                            <label class="mb-2 block text-sm font-semibold text-gray-800">{{ trans("messages.$filter") }}</label>
                            <select name="{{ $filter }}" class="form-select w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                    ...
                    <div class="flex items-end gap-2 pt-6">
                        <button type="submit" class="rounded-md bg-orange-600 px-6 py-2 text-white shadow transition hover:bg-orange-700">Apply</button>
                        <a href="{{ route('products.category', $category->slug) }}" class="rounded-md border border-gray-300 bg-white px-6 py-2 text-center text-gray-700 shadow-sm transition hover:bg-gray-50">Reset</a>
                    </div>
                </form>
            </div>
        </div>
            ...
            @if ($products->isNotEmpty())
                <div class="mb-8">
                    ...
                    <h2 class="text-3xl font-light text-gray-900" data-total-products="{{ $products->total() }}">
                        Showing <span id="product-count">{{ $products->count() }}</span> of {{ $products->total() }} Products
                    </h2>
                </div>
                <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                    <div role="table" class="min-w-full">
                        <div role="rowheader" class="grid grid-cols-[minmax(300px,_3fr)_repeat(4,_1fr)_minmax(80px,_auto)] border-b-2 border-gray-200 bg-gray-50 text-xs font-semibold uppercase tracking-wider text-gray-600">
                        ...
                        <div id="product-table-body" role="rowgroup">
                            @foreach ($products as $prod)
                                <div role="row" class="group relative grid grid-cols-[minmax(300px,_3fr)_repeat(4,_1fr)_minmax(80px,_auto)] items-center border-b border-gray-200 transition-colors duration-300 last:border-b-0 hover:bg-orange-50/50">
                                    <div class="whitespace-nowrap py-4 px-4">
                                        <h4 class="truncate text-base font-bold text-gray-900">{{ $prod->unique_id }}</h4>
                                        <p class="truncate text-sm text-gray-500">{{ $prod->getTranslation('name', $lang) }}</p>
                                    </div>
                                    <div class="whitespace-nowrap py-4 px-4 text-sm text-gray-700">{{ $prod->power }}</div>
                                    <div class="whitespace-nowrap py-4 px-4 text-sm text-gray-700">{{ $prod->poles }}</div>
...
{{-- ADDED: New Section with Featured Image --}}
    ...
    <x-container>
        <div class="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-0">
            ...
            <div class="animate-on-scroll" data-anim="clip-reveal">
                <img src="{{ asset('assets/images/catelog-sec-img.jpg') }}" alt="Engineering catalog and motor parts" class="object-cover shadow-2xl">
            </div>

            {{-- 2. Overlapping Text Card Column --}}
            <div class="relative z-10 lg:-ml-20">
                <div class="animate-on-scroll rounded-lg bg-white p-10 shadow-2xl md:p-12" data-anim="fade-up">
                    ...
                    <p class="mt-4 text-lg text-gray-600">
                        Our comprehensive catalogues and brochures provide in-depth technical data, performance charts, and application guides for our entire product range.
                    </p>
                    <div class="mt-8">
                        <a href="{{ url('/brochures') }}" class="inline-block bg-[#f06425] px-8 py-3 text-base font-medium text-white shadow-lg transition-transform duration-300 hover:scale-105 hover:bg-[#e14e0f]">
                            Explore Catalogues
                        </a>
                    </div>
                </div>
    ...
    <x-container>
        ...
        <div class="custom-scrollbar animate-on-scroll -mx-4 overflow-x-auto pb-8">
            ...
            @foreach ($category->childs->where('is_active', 1) as $cat)
                <a href="{{ route('products.category',['category_slug' => $cat->slug]) }}" class="group relative block h-[28rem] w-80 flex-shrink-0 overflow-hidden rounded-lg shadow-lg md:w-96">
                    <img src="{{ uploaded_asset($cat->image) }}" alt="{{ $cat->getTranslation('name', $lang) }}" class="absolute inset-0 h-full w-full object-cover transition-transform duration-500 ease-in-out group-hover:scale-105">

        ...
        <div class="col-lg-8 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6 d-flex">Create Manual Section</h5>
                    <span>{{ $manual->title }}</span>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" method="POST" action="{{ route('manual-sections.store') }}">
                        @csrf
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="text" placeholder="Enter..." value="{{ old('title') }}" id="title"
                                    ...
                                    <input type="hidden" name="manual_id"  id="manual_id" value="{{$manual->id}}">
                                @error('title')
                        ...
                        <div class="form-group mb-0 text-right">
                            ...
                            <a href="{{ route('manual-sections.all',['id'=>$manual->id]) }}" class="btn btn-md btn-cancel">{{trans('messages.cancel')}}</a>
                        </div>
                    </form>
                </div>
            </div>

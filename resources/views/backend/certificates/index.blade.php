        ...
        <div class="col-lg-12 mx-auto">
            ...
            <div class="aiz-titlebar text-left mt-2 mb-3">
                <div class="row align-items-center">
                    <div class="col-md-6">
                        <h1 class="h3">{{ trans('messages.all').' '.trans('messages.certificates') }}</h1>
                    </div>
                    <div class="col-md-6 text-md-right">
                        <a href="{{ route('certificate.create') }}" class="btn btn-primary">
                            <span>{{ trans('messages.add_new').' '.trans('messages.certificate') }}</span>
                        </a>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($certificates as $key => $cert)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('sections.all',['id' => $cert->id]) }}" class="btn btn-cancel">Manage Sections</a>
                                        </td>

                                        <td class="text-right">
                                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                                href="{{ route('certificate.edit', ['id' => $cert->id, 'lang' => 'en']) }}" title="Edit">
                                                <i class="las la-edit"></i>
                                            </a>
                                            <a href="#"
                                                class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                                data-href="{{ route('certificate.delete', $cert->id) }}" title="Delete">
...
@section('script')
    <script>
        function update_published(el) {
            ...
            $.post('{{ route('certificate.update-status') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {

...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{trans('messages.all') . ' '.trans('messages.industries')}}</h1>
        </div>
        <div class="col-md-6 text-md-right">
            <a href="{{ route('industries.create') }}" class="btn btn-primary">
                <span>{{trans('messages.add_new') . ' '.trans('messages.industry')}}</span>
            </a>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-header d-block d-md-flex">
        <h5 class="mb-0 h6 mr-4">{{ trans('messages.industries') }}</h5>

        <form class="" id="sort_industries" action="" method="GET" style="width: 100%">

            <div class="row gutters-5">
                {{-- <div class="col-md-4">
                    <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" data-live-search="true"
                        name="industry" id="" data-selected={{ $industry }}>
                ...
                <div class="col-md-2">
                    ...
                    <a href="{{ route('industries.index') }}" class="btn btn-warning">Reset</a>
                </div>
            </div>
            ...
            <tbody>
                @foreach($industries as $key => $industry)
                    <tr>
                        ...
                        <td>{{ $industry->getTranslation('name') }}</td>
                        {{-- <td>
                            @php
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="update_status(this)" value="{{ $industry->id }}" <?php if($industry->is_active == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
                        <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('industries.edit', ['id'=>$industry->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ trans('messages.edit') }}">
...
@section('script')
    <script type="text/javascript">
        function update_status(el){
            ...
            $.post('{{ route('industries.status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ trans('messages.industry').' '. trans('messages.status').' '. trans('messages.updated_msg') }}');
                }
                else{
